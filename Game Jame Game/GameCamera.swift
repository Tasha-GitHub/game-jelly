...
class GameCamera: SKCameraNode {
    func setConstrains(with scene: SKScene, and frame: CGRect, to node: SKNode?){
        let scaleSize = CGSize(width: scene.size.width * xScale, height: scene.size.height * yScale)
        let boardContentRect = frame
        
        let xInset = min(scaleSize.width/2, boardContentRect.width/2)
        let yInset = min(scaleSize.height/2, boardContentRect.height/2)
